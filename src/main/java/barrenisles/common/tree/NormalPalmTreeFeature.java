...
import barrenisles.common.util.feature.FeatureUtil;
...
public class NormalPalmTreeFeature extends PalmTreeFeature {
    ...
    public boolean place(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
        ...
        if (FeatureUtil.goesBeyondWorldSize(world, pos.getY(), height)) {
            return false;
        }

        if (!FeatureUtil.isBBAvailable(world, pos, height)) {
            return false;
        }
        ...
        for (int j1 = 0; j1 < height + 2; j1++) {
            ...
            if (TreeFeature.isAirOrLeaves(world, logPos)) {
