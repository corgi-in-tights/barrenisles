...
public class GoldVaseContainer extends Container {
	...
	private final int containerRows = 45;
	private final IInventory container;
	
	public GoldVaseContainer(int windowId, PlayerInventory playerInv, TileEntity tileEntity) {
		this(BarrenIslesContainers.gold_vase_container.get(), windowId, playerInv, tileEntity, new Inventory(45));
	}
	
	public GoldVaseContainer(int windowId, PlayerInventory playerInv, TileEntity tileEntity, IInventory inventory) {
		this(BarrenIslesContainers.gold_vase_container.get(), windowId, playerInv, tileEntity, inventory);
	}
	
	public GoldVaseContainer(ContainerType<?> menuType, int windowId, PlayerInventory playerInv, TileEntity tileEntity, IInventory inventory) {
	...
	public GoldVaseContainer(final int windowId, final PlayerInventory player_inv, final PacketBuffer data) {
        this(BarrenIslesContainers.gold_vase_container.get(), windowId, player_inv, getTileEntity(player_inv, data), new Inventory(45));
    }

	@Override
	public boolean stillValid(PlayerEntity playerIn) {
		return stillValid(
			canInteractWithCallable, playerIn, BarrenIslesBlocks.gold_vase.get());
	}
