...
public class VaseContainer extends Container {
    ...
    public static VaseContainer createContainerServerSide(int windowID, PlayerInventory playerInventory, VaseContents vaseContents) {
        return new VaseContainer(windowID, playerInventory, vaseContents);
    }

    public static VaseContainer createContainerClientSide(int windowID, PlayerInventory playerInventory, net.minecraft.network.PacketBuffer extraData) {
        VaseContents vaseContents = VaseContents.createForClientSideContainer(VaseTileEntity.NUMBER_OF_SLOTS);
        return new VaseContainer(windowID, playerInventory, vaseContents);
    }
    ...
    private static final int TE_INVENTORY_SLOT_COUNT = VaseTileEntity.NUMBER_OF_SLOTS;

    public VaseContainer(int windowID, PlayerInventory playerInventory, VaseContents vaseContents) {
        super(BarrenIslesContainers.vase_container.get(), windowID);
        if (BarrenIslesContainers.vase_container == null)
        ...
        for(int x = 0; x < 3; ++x) {
            this.addSlot(new Slot(vaseContents, x, 62 + x * 18, 20));
        }
